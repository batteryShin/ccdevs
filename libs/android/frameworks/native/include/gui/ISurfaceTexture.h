...
public:
   ...
   // dequeue buffer.
	...
	//2012-09-07 Jinwon-Jung(jw.jung@lge.com) [gk/VideoNewFeature]PreviewFrame[START]
    // setDestMargin indicates the margin of each direction to be rendered on window
    // each margin should be normalized.( 0 ~1.0 )
#if defined (LIVE_ZOOMING) || defined (FINGER_TIP_SEEK)
    virtual status_t setDestMargin(float leftMargin, float topMargin, float rightMargin, float bottomMargin) = 0;

    // force surfaceflinger to render the this surfacetexture, if true.
    // otherwise h/w composer choose whether this surface to be rendered by overlay or surfaceflinger.
    virtual status_t enableOpenGLRendering(int enable) = 0;
    virtual status_t setCropForPlayer(const Rect& reg) = 0;
#endif
	//2012-09-07 Jinwon-Jung(jw.jung@lge.com) [gk/VideoNewFeature]PreviewFrame[END]
