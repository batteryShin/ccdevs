...
private:
    ...
    int dispatchSetDestMargin(va_list args);
    int dispatchEnableOpenGLRendering(va_list args);
    int dispatchSetCropForPlayer(va_list args);
#endif
...
protected:
	...
	//2012-09-07 Jinwon-Jung(jw.jung@lge.com) [gk/VideoNewFeature]PreviewFrame[START]
#if defined (LIVE_ZOOMING) || defined (FINGER_TIP_SEEK)
    virtual int setDestMargin(float leftMargin, float topMargin, float rightMargin, float bottomMargin);
    virtual int enableOpenGLRendering(int enable);
    virtual int setCropForPlayer(Rect const* rect);
#endif
	//2012-09-07 Jinwon-Jung(jw.jung@lge.com) [gk/VideoNewFeature]PreviewFrame[END]
